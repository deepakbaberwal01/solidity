...
void ContractCompiler::appendCallPreventer(ContractDefinition const& _contract)
{
	...
	// This has to be as complicated as possible and only used once
	// so that the optimizer will not change it.
	u160 id = u160(u256(keccak256(_contract.fullyQualifiedName())));
	// Prevent it from being used with anything else than PUSH20
	id |= (u160(1) << 159);
	m_context << id << Instruction::ADDRESS << Instruction::EQ;
	m_context.appendConditionalRevert();
}
